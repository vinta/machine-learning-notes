...
val otherFields = columnNames.tail.map(field => StructField(field, DoubleType, nullable = false))
StructType(firstField :: otherFields)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    var primaryColumns = new ListBuffer[Column]()
    var workingColumns = new ListBuffer[Column]()
    var otherColumns = new ListBuffer[Column]()

    columnNames.foreach((columnName: String) => {
      val column = new Column(columnName)
      if (primaryActivities.exists(columnName.startsWith)) primaryColumns += column
      else if (workingActivities.exists(columnName.startsWith)) workingColumns += column
      else if (otherActivities.exists(columnName.startsWith)) otherColumns += column
    })

    (primaryColumns.toList, workingColumns.toList, otherColumns.toList)
  }
